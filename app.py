...
def findPage(itemToFind, store_list = None, filterOption = None, isAcsending = None, price_from = None, price_to = None, currency = None):
                ...
                if row["price"] != "" and row["price"].find('price') == -1 and row["price"].find(",") == -1:
                    if currency == "azn":     
                        if row["site"] == "tapaz":
                            ...
                            row["price"] = str(truncate(int(row["price"].replace(" ", "")) * 1.7, 2)) + "₼"
                    else: # us dollars
                        if row["site"] == "tapaz":
                            row["price"] = str(truncate(int(row["price"].replace(" ", "")) / 1.7, 2)) + "$"
                        else:
    ...
    if filterOption.find('shipping') == -1 and filterOption.find('No') == -1:
        ls_all = filterPrices(ls_all, price_from, price_to)[:]

    if filterOption != None and filterOption.find('No') == -1:
    ...
    return render_template("./public/base.html", isSearching = True, itemsAmazon = ls_amazon, itemsTapAz = ls_tapaz)
...
def process():
    ...
    if request.method == "POST":
        if request.form['submit_button'] == 'Search':
            return redirect(url_for("findPage", itemToFind = request.form['search_text']), store_list = "None", filterOption = "None", isAcsending = "None", price_from = "None", price_to = "None", currency = "None")
